...
public:
    ...
    int GetPrev(int i){return Prev->Get(i);}
...
int SubTask::Solve(Path& Otvet) { 
    ...
    if(Fir>=Sec || Judge ==0) { // second case
...
void Task::ReadFile() {
    ...
    FILE* fp1;
    //errno_t err;
    if ((fp1 = fopen("test20.txt", "r")) == NULL){
    //if ((err= fopen_s(&fp1,"test1.txt", "r")) == 0) {
        printf("Cannot open test file\n");
        exit(0);
    }
    char c;
    c = fgetc(fp1);
    int i = 1; int j = 1; int count = 0; int val;
    while (c != EOF) {
        while (1) {
            char* tmpstr;
            tmpstr = new char[10];
            ...
            val = atoi(tmpstr);
            delete tmpstr;
            Matr[MakeIndex(i,j)] = val;
            j++; count = 0;
            if (c == '\n') { c = fgetc(fp1); break; }//need final enter
            c = fgetc(fp1);
        }
        i++; j = 1;
    }
...
int main()
{
    ...
    Task T; //T.InitRnd(); 
    T.ReadFile(); 
    T.InitFirst(); cout<<"#0"<<endl;cout << T << endl;
    T.Run();
    end_t = clock();
    ...
    SubTask* First;
    //SubTask* FirstRight;
    First= T.ExtractFirst(); cout<<"#0 Original task with reduction"<<endl; cout<<*First<<endl;

    T.SolveSubTask(First,1);
    cout<<"----------------"<<endl;
    cout<<"nKol= "<<T.GetnKol()<<" nOpt= "<<T.GetnOpt()<<endl;
    cout<<"nKol after Del= "<<T.DelTail(T.GetnOpt())<<endl;
    cout<<"BestNext= "<<T.BestNext()<<endl;
    cout<<"----------------"<<endl;
    cout<<"----------------"<<endl;

    SubTask* NewTask = T.GetZadachi(T.BestNext());
    T.SolveSubTask(NewTask,T.BestNext());
    cout<<"----------------"<<endl;
    cout<<"nKol= "<<T.GetnKol()<<" nOpt= "<<T.GetnOpt()<<endl;
    cout<<"nKol after Del= "<<T.DelTail(T.GetnOpt())<<endl;
    ...
    */
    ...
    /*
    int nXdel,nYdel,Best;
    Best = First->BestNull(nXdel,nYdel);
    FirstRight = First->MakeRight(nXdel,nYdel,Best);
    cout<<"#00"<<endl;cout<<*First<<endl;
    cout<<"#01"<<endl;cout<<*FirstRight<<endl;

    SubTask* SecondRight;
    Best = FirstRight->BestNull(nXdel,nYdel);
    SecondRight = FirstRight->MakeRight(nXdel,nYdel,Best);
    cout<<"#010"<<endl;cout<<*FirstRight<<endl;
    cout<<"#011"<<endl;cout<<*SecondRight<<endl;

    SubTask* ThirdRight;
    Best = SecondRight->BestNull(nXdel,nYdel);
    cout<<nXdel<<" "<<nYdel<<endl;
    ThirdRight = SecondRight->MakeRight(nXdel,nYdel,Best);
    cout<<"#0110"<<endl;cout<<*SecondRight<<endl;
    cout<<"#0111"<<endl;cout<<*ThirdRight<<endl;

    Path Otvet; Otvet.InitNull();
    ThirdRight->Solve(Otvet);
    cout<<"Otvet: ";
    for(int i=1;i<=nDimAll;i++)
        cout<<Otvet.Get(i)<<"";
    */
