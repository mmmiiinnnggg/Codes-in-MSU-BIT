...
int main()
{
    ...
    if((fp1=fopen("Amat12.m","r"))==NULL){
        printf("Cannot open AMat4\n");
        exit(0);
    }
    if((fp2=fopen("Lmat12.m","w+"))==NULL){
        printf("Cannot open LMat4Up\n");
        exit(0);
    }
    if((fp3=fopen("Umat12.m","w+"))==NULL){
        printf("Cannot open UMat4Up\n");
        exit(0);
    }
    ...
    for(int i=1; i<=n; i++){
        for(int j=1; j<n; j++){
            ...
            else fprintf(fp2,"%f ",LU[i][j].im);
        }
        if(i!=n) fprintf(fp2,"%d;\n  ",0);
        else fprintf(fp2,"%d]);",0);
    }
    ...
    for(int i=1; i<=n; i++){
        for(int j=1; j<n; j++){
            if(i<=j) fprintf(fp3,"%f ",LU[i][j].re);
            else fprintf(fp3,"%d ",0);

        }
        if(i!=n) fprintf(fp3,"%f;\n  ",LU[i][n].re);
        else fprintf(fp3,"%f],[",LU[n][n].re);
    }

    for(int i=1; i<=n; i++){
        for(int j=1; j<n; j++){
            if(i<=j) fprintf(fp3,"%f ",LU[i][j].im);
            else fprintf(fp3,"%d ",0);
        }
        if(i!=n) fprintf(fp3,"%f;\n  ",LU[i][n].im);
        else fprintf(fp3,"%f]);",LU[n][n].im);
    }

    cout<<"Success! Please check the answer in Amat12.m, Lmat12.m and Umat12.m"<<endl;
