...
int main()
{
    ...
    if((fp1=fopen("Amat3.m","r"))==NULL){
        printf("Cannot open AMat3\n");
        exit(0);
    }
    if((fp2=fopen("bvec3.m","r+"))==NULL){
        printf("Cannot open bvec3\n");
        exit(0);
    }
    if((fp3=fopen("xvec3.m","w+"))==NULL){
    ...
    if( ((fp4=fopen("Lmat3.m","r"))!=NULL) && ((fp5=fopen("Umat3.m","r"))!=NULL)){
        recognizeA(LU,fp4);
        double L[MAX_n];
        for(int i=1; i<=n; i++)
            for(int j=1; j<i; j++)
        ...
        if((fp4=fopen("Lmat3.m","w+"))==NULL){
            cout<<"Cannot open Lmat3\n";
            exit(0);
        }

        if((fp5=fopen("Umat3.m","w+"))==NULL){
    ...
    cout<<"Success! Please check the answer in Amat3.m, bvec3.m and xvec3.m"<<endl;
