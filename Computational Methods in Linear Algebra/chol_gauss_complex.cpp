...
void SolveSystem(){
    ...
    for(int i=n; i>=1; i--){
        complex tmp(0,0);
        for(int j=i+1; j<=n; j++)
            tmp=tmp+C[makeIndex(j,i)].conju()*x[j];
        x[i]=(y[i]-tmp)/C[makeIndex(i,i)].conju();
    }
}
...
int main()
{
    ...
    if((fp4=fopen("Cmat13.m","r+"))!=NULL){
        complex CC[MAX_n][MAX_n];
        recognizeReA(CC,fp4);
        recognizeImA(CC,fp4);
        for(int i=1; i<=n; i++)
            for(int j=1; j<=i; j++){
    ...
    cout<<"Success! Please check the answer in Amat13.m, bvec13.m, Cmat13.m and xvec13.m"<<endl;
