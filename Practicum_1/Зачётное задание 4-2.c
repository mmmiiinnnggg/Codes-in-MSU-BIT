...
void difficult_sort_insert(int Y[],int l,int r)
{
	int i,j,mid;
	i=l;j=r;
	mid=Y[(l+r)/2];
	...
	{
		countop++;
		while((Y[i]<mid)&&(i<r)) {countop+=2;i++;}//must Y[i]<mid
	    while((Y[j]>mid)&&(j>l)) {countop+=2;j--;}//must Y[j]>mid
	    if(i<=j)
	    {
			countop++;
			swapelem(Y+i,Y+j);
	...
	if(j>l) {countop++;difficult_sort_insert(Y,l,j);}
	if(i<r) {countop++;difficult_sort_insert(Y,i,r);}
}


int main()
{
	int a[4][7],b[4][7],n[4]={10,20,50,100},i;
	int p,q,k;
	srand(time(NULL));
...
////////// The difficult sort //////////////////////////////////////////
    ...
    int l=0,r=n[k]-1;
    sum=0;countop=0;
    b[k][0]=ntmp;//THE NUMBER OF MATRIX
	
	for(i=0;i<ntmp;i++)
	  X[i]=first[i];
	difficult_sort_insert(X,l,r);
	b[k][1]=countop;
	countop=0;
    ...
    for(j=1;j<6;j++)
      sum=sum+b[k][j];
    b[k][6]=sum/5;//CALCULATE THE AVERAGE NUMBER
}

//////////////////////////////////////////////////////////////
/////////// Print out the Table1 ////////////////////////////
